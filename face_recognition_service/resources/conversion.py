...
class Conversion(Resource):
    def post(self):
        ...
        filename = os.path.join(app.config['convert_faces_dir'], '{}.cvttmp'.format(uuid.uuid1()))

        try:
            with open(filename, 'wb') as tmp:
                tmp.write(args['face_image'].read())

            image = engine.load_image_file(filename)
        finally:
            os.unlink(filename)
        ...
        np_file = os.path.join(app.config['convert_faces_dir'], '{}.npy'.format(uuid.uuid1()))
